		...
		AbstractGeoWaveBasicVectorIT
{
	...
	@Test
	public void testHailTornadoIntersectsJoin() {
		...
		String sqlHail = "select hail.* from hail, tornado where geomIntersects(hail.geom,tornado.geom)";
		String sqlTornado = "select tornado.* from hail, tornado where geomIntersects(hail.geom,tornado.geom)";
		...
		Assert.assertTrue((hailIndexedCount == hailBruteResults.count()));
		Assert.assertTrue((tornadoIndexedCount == tornadoBruteResults.count()));
		Assert.assertTrue(
	...
	private long runBruteForceJoin(
		...
		long hailBruteCount = hailBruteResults.count();
		...
		long tornadoBruteCount = tornadoBruteResults.count();
		long dur = (System.currentTimeMillis() - mark);
